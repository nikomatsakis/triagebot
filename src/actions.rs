...
impl<'a> Action for Step<'a> {
    async fn call(&self) -> String {
        ...
        let all_queries = self
            .actions
            .iter()
            .flat_map(|Query { repos, queries }| repos.iter().map(move |repo| (repo, queries)))
            .flat_map(|(repo, queries)| {
                queries
                    .iter()
                    .map(move |QueryMap { name, query }| (repo, name, query))
            });
        for (repo, name, query) in all_queries {
